...
class SplitManageCategoriesActivity : AppCompatActivity() {

    private val categoryDao: SplitCategoryDao by lazy {
        AppDatabase.getInstance(this).splitCategoryDao()
    }

    private val syncManager: SplitSyncManager by lazy {
        SplitSyncManager.getInstance(this)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = getString(R.string.split_manage_categories_title)
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    if (existing != null) {
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@SplitManageCategoriesActivity, R.string.category_already_exists, Toast.LENGTH_SHORT).show()
                        }
                        return@launch
                    }
                    val newCategory = SplitCategory(
                        name = name,
    ...
    private fun showEditDialog(category: SplitCategory) {
    ...
    private fun showDeleteDialog(category: SplitCategory) {
        AlertDialog.Builder(this)
    ...
    private class CategoryListAdapter(
        private val onEdit: (SplitCategory) -> Unit,
        private val onDelete: (SplitCategory) -> Unit
    ) : RecyclerView.Adapter<CategoryListAdapter.ViewHolder>() {

        private var categories: List<SplitCategory> = emptyList()

        fun setCategories(list: List<SplitCategory>) {
            categories = list
            notifyDataSetChanged()
        }

        fun getCategories(): List<SplitCategory> = categories
